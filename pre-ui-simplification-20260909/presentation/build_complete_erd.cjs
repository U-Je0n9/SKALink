...
assert.equal(summary.tables,15);assert.equal(summary.columns,142);assert.equal(summary.refs,25);assert.equal(summary.enums,6);
assert.equal(summary.indexes,raw.split('\n').filter(l=>/^    \(/.test(l)).length);
vm.runInContext(`for(const r of refs){if(!schema[r.from].fields.some(f=>f.name===r.field)||!schema[r.to].fields.some(f=>f.name===r.target))throw Error('Invalid FK')}for(const n of ['question_tags','material_reads','post_user_states'])if(schema[n].fields.filter(f=>f.pk).length!==2)throw Error('Missing composite PK');for(const t of Object.values(schema))if(t.y+t.h>bottom)throw Error('Table overflow')`,c);
const svg=document.querySelector('#stage').innerHTML;assert.equal((svg.match(/class="edge"/g)||[]).length,25);assert.equal((svg.match(/data-column=/g)||[]).length,142);assert.equal((svg.match(/class="entity"/g)||[]).length,15);assert(!/NaN|undefined/.test(svg));
fs.writeFileSync(path.join(__dirname,'SKALog-ERD.html'),html);fs.writeFileSync(path.join(__dirname,'erd-exports/SKALog-ERD-complete.svg'),svg);
