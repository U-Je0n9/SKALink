const KEY = "skala-knowledge-v5";

const initial = {
  users: [
    { id: 1, loginId: "student1", password: "1234", pCode:"P001", email:"haneul@skala.example", name: "김하늘", role: "STUDENT", classNumber: 1, floor: 4, status:"ACTIVE" },
    { id: 2, loginId: "student4", password: "1234", pCode:"P104", email:"doyun@skala.example", name: "박도윤", role: "STUDENT", classNumber: 4, floor: 4, status:"ACTIVE" },
    { id: 5, loginId: "student6", password: "1234", pCode:"P206", email:"garam@skala.example", name: "윤가람", role: "STUDENT", classNumber: 6, floor: 5, status:"ACTIVE" },
    { id: 3, loginId: "professor", password: "1234", name: "백정열", role: "PROFESSOR", classNumber: null, floor: null, status:"ACTIVE" },
    { id: 6, loginId: "practice", password: "1234", name: "김경난", role: "PROFESSOR", classNumber: null, floor: null, status:"ACTIVE" },
    { id: 7, loginId: "professor2", password: "1234", name: "임성열", role: "PROFESSOR", classNumber: null, floor: null, status:"ACTIVE" },
    { id: 4, loginId: "admin", password: "1234", name: "최관리", role: "ADMIN", classNumber: null, floor: null, status:"ACTIVE" }
  ],
  roster: [
...
export const api = {
  async login(loginId, password) { const d=load(); const u=d.users.find(x=>x.loginId===loginId&&x.password===password); if(!u) throw new Error("아이디 또는 비밀번호를 확인해주세요."); if(u.status==="PENDING")throw new Error("관리자 승인 대기 중인 계정입니다."); if(u.status==="REJECTED"||u.status==="INACTIVE")throw new Error("사용할 수 없는 계정입니다."); d.sessionUserId=u.id; save(d); return delay(u); },
  async register(input){const d=load();if(!/^P\d{3}$/.test(input.pCode))throw new Error("고유번호는 P와 숫자 3자리로 입력해주세요.");if(d.users.some(x=>x.loginId===input.loginId||x.email===input.email||x.pCode===input.pCode))throw new Error("이미 가입된 아이디, 이메일 또는 고유번호입니다.");const matched=d.roster.some(r=>r.pCode===input.pCode&&r.name===input.name&&r.classNumber===Number(input.classNumber)&&r.email.toLowerCase()===input.email.toLowerCase()),id=Math.max(...d.users.map(x=>x.id),0)+1,u={id,...input,classNumber:Number(input.classNumber),floor:Number(input.classNumber)<=5?4:5,role:"STUDENT",status:matched?"ACTIVE":"PENDING"};d.users.push(u);save(d);return delay({user:u,autoApproved:matched});},
